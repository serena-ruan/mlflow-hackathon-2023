...
Python only. For Java, see `java_api`. For R, see `R-api`.
...
For more options, see `quickstart_drilldown_install`.

## Add MLflow tracking to your code {#add-mlflow-tracking-to-your-code}
...
your run (see list at `automatic-logging`). For instance, the following
autologs a **scikit-learn** run:
...
-   For more details on the explicit logging API, see
    `tracking_logging_functions`.

## View MLflow runs and experiments {#view-mlflow-runs-and-experiments}
...
![Screenshot of MLflow tracking UI](../../../static/images/quickstart/quickstart_ui_screenshot.png)
...
![Screenshot of MLflow tracking UI details page](../../../static/images/quickstart/quickstart_ui_detail_page.png)
...
![Screenshot of MLflow tracking UI metric history](../../../static/images/quickstart/quickstart_ui_metric_history.png)
...
![Screenshot of MLflow tracking UI chart view](../../../static/images/quickstart/quickstart_ui_chart_view.png)

For more details on the tracking UI, see `tracking`.
...
details, see `tracking_server`.

### Use MLflow with a Databricks workspace {#use-mlflow-with-a-databricks-workspace}
...
![Screenshot showing location of URL in Databricks Workspace Configuration page](../../../static/images/quickstart/quickstart_databricks_workspace_url.png)
...
For more on using MLflow with Databricks, see [Databricks' documentation
on MLflow](https://docs.databricks.com/mlflow/index.html).

## Store a model in MLflow {#store-a-model-in-mlflow}
...
-   An **MLModel** file in YAML format specifying the model's **flavor**
    (or **flavors**), dependencies, signature (if supplied), and
    important metadata;
...
(`mlflow.start_run` API reference)
...
If you've not called `set_tracking_uri` or set the `MLFLOW_TRACKING_URI`
environment variable to point to a remote tracking server, this
**model** directory will be under the **mlruns** directory.

For more information, including a list of supported model flavors and
storing your own flavor, see `models_built-in-model-flavors`.

## Load a model from a specific training run for inference {#load-a-model-from-a-specific-training-run-for-inference}
...
![Screenshot showing location of run ID in tracking UI](../../../static/images/quickstart/quickstart_run_id.png)
...
detail in `model-enviroment-management`.
